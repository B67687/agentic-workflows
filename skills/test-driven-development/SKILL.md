...
Load the test pyramid reference (sizes, decision guide, resource model) as L3:
`bash ./scripts/skill-toolset.sh resource test-driven-development references/test-pyramid.md`

## Writing Good Tests

### Test State, Not Interactions

Focus on verifying the output/behavior of code, not the internal implementation details
or whether specific functions were called correctly. Implementation-first tests are
brittle --- they break when you refactor even if the behavior stays the same.

Load test examples (L3) showing state vs. interaction testing:
`bash ./scripts/skill-toolset.sh resource test-driven-development references/test-examples.md`
