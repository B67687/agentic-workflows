...
description: Hardens code against vulnerabilities. Use when handling user input, authentication, data storage, or external integrations. Use when building any feature that accepts untrusted data, manages
...
> See `references/owasp-top-10.md` (L3) for categorized prevention guidance with
> code examples (injection, auth, XSS, access control, misconfiguration, sensitive data).
> Load with:
> `bash ./scripts/skill-toolset.sh resource security-and-hardening references/owasp-top-10.md`
...
> See `references/security-checklist.md` (L3) for the full checklist covering OWASP
> Top 10, boundary enforcement, and pre-commit verification. Load with:
> `bash ./scripts/skill-toolset.sh resource security-and-hardening references/security-checklist.md`
