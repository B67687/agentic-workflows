...
(function() {
  ...
  function applyHighlight() {
    ...
    // Apply toggling to nodes: if a node filter is active, dim non-matching nodes;
    // if only an edge filter is active, dim ALL nodes.
    NODES.forEach(function(n) {
      const match = hasNodeFilter ? n.group === highlightGroup : false;
      const shouldDim = anyFilter && !match;
    ...
    // Apply toggling to edges: if an edge filter is active, dim non-matching edges;
    // if only a node filter is active, dim ALL edges.
    EDGES.forEach(function(e) {
      if (e.hidden) return;
      const eType = edgeLegendType(e);
      const matchEdge = hasEdgeFilter ? eType === highlightEdge : false;
      const shouldDim = anyFilter && !matchEdge;
      if (!e._origColor) {
